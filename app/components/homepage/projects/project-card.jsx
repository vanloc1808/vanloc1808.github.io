"use client";

import * as React from 'react';
import GlowCard from '../../helper/glow-card';

function ProjectCard({ project, identifier }) {
  return (
    <GlowCard identifier={identifier}>
      <div className="relative w-full rounded-lg border border-gray-300 dark:border-[#1b2c68a0] bg-gradient-to-r from-gray-100 to-gray-200 dark:from-[#0d1224] dark:to-[#0a0d37]">
        {/* Top gradient line */}
        <div className="flex flex-row">
          <div className="h-[1px] w-full bg-gradient-to-r from-transparent via-pink-500 to-violet-600"></div>
          <div className="h-[1px] w-full bg-gradient-to-r from-violet-600 to-transparent"></div>
        </div>

        {/* Title */}
        <div className="relative px-4 py-3 lg:px-8 lg:py-5">
          <div className="absolute top-1/2 -translate-y-1/2 flex flex-row space-x-1 lg:space-x-2">
            <div className="h-2 w-2 lg:h-3 lg:w-3 rounded-full bg-red-400" />
            <div className="h-2 w-2 lg:h-3 lg:w-3 rounded-full bg-orange-400" />
            <div className="h-2 w-2 lg:h-3 lg:w-3 rounded-full bg-green-200" />
          </div>
          <p className="ml-3 text-center text-base text-[#16f2b3] lg:text-xl">
            {project.name}
          </p>
        </div>

        {/* Code block */}
        <div className="overflow-hidden border-t-[2px] border-indigo-900 dark:border-indigo-900 px-4 py-4 lg:px-8 lg:py-8">
          <code className="font-mono text-xs md:text-sm lg:text-base">
            <div className="blink">
              <span className="mr-2 text-pink-600 dark:text-pink-500">const</span>
              <span className="mr-2 text-gray-800 dark:text-white">project</span>
              <span className="mr-2 text-pink-600 dark:text-pink-500">=</span>
              <span className="text-gray-500 dark:text-gray-400">{'{'}</span>
            </div>

            <div>
              <span className="ml-4 lg:ml-8 mr-2 text-gray-800 dark:text-white">name:</span>
              <span className="text-gray-500 dark:text-gray-400">'</span>
              <span className="text-amber-600 dark:text-amber-300">{project.name}</span>
              <span className="text-gray-500 dark:text-gray-400">',</span>
            </div>

            <div className="ml-4 lg:ml-8 mr-2">
              <span className="text-gray-800 dark:text-white">tools:</span>
              <span className="text-gray-500 dark:text-gray-400">[</span>
              {project.tools.map((tag, i) => (
                <React.Fragment key={i}>
                  <span className="text-amber-600 dark:text-amber-300">'{tag}'</span>
                  {i < project.tools.length - 1 && (
                    <span className="text-gray-500 dark:text-gray-400">, </span>
                  )}
                </React.Fragment>
              ))}
              <span className="text-gray-500 dark:text-gray-400">],</span>
            </div>

            <div>
              <span className="ml-4 lg:ml-8 mr-2 text-gray-800 dark:text-white">myRole:</span>
              <span className="text-orange-600 dark:text-orange-400">{project.role}</span>
              <span className="text-gray-500 dark:text-gray-400">,</span>
            </div>

            <div className="ml-4 lg:ml-8 mr-2">
              <span className="text-gray-800 dark:text-white">Description:</span>
              <span className="text-cyan-600 dark:text-cyan-400"> {project.description}</span>
              <span className="text-gray-500 dark:text-gray-400">,</span>
            </div>

            {project.code && (
              <div className="ml-4 lg:ml-8 mr-2">
                <span className="text-gray-800 dark:text-white">Code:</span>
                <span className="text-cyan-600 dark:text-cyan-400">
                  {' '}
                  <a href={project.code} target="_blank" rel="noreferrer">
                    {project.code}
                  </a>
                </span>
                <span className="text-gray-500 dark:text-gray-400">,</span>
              </div>
            )}

            {project.backend_code && (
              <div className="ml-4 lg:ml-8 mr-2">
                <span className="text-gray-800 dark:text-white">Backend Code:</span>
                <span className="text-cyan-600 dark:text-cyan-400">
                  {' '}
                  <a href={project.backend_code} target="_blank" rel="noreferrer">
                    {project.backend_code}
                  </a>
                </span>
                <span className="text-gray-500 dark:text-gray-400">,</span>
              </div>
            )}

            {project.frontend_code && (
              <div className="ml-4 lg:ml-8 mr-2">
                <span className="text-gray-800 dark:text-white">Frontend Code:</span>
                <span className="text-cyan-600 dark:text-cyan-400">
                  {' '}
                  <a href={project.frontend_code} target="_blank" rel="noreferrer">
                    {project.frontend_code}
                  </a>
                </span>
                <span className="text-gray-500 dark:text-gray-400">,</span>
              </div>
            )}

            {project.demo && (
              <div className="ml-4 lg:ml-8 mr-2">
                <span className="text-gray-800 dark:text-white">Demo:</span>
                <span className="text-cyan-600 dark:text-cyan-400">
                  {' '}
                  <a href={project.demo} target="_blank" rel="noreferrer">
                    {project.demo}
                  </a>
                </span>
                <span className="text-gray-500 dark:text-gray-400">,</span>
              </div>
            )}

            <div>
              <span className="text-gray-500 dark:text-gray-400">{'};'}</span>
            </div>
          </code>
        </div>
      </div>
    </GlowCard>
  );
}

export default ProjectCard;