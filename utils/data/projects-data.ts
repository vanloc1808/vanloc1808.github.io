...
export const projectsData: MultilingualProjectsData = {
  en: [
    {
      id: 11,
      name: 'Doodle Duel (Jankenpon): The Escalation Game',
      description:
        "This is our participation to Gemini 3 Hackathon Tokyo (Gemini 3 ハッカソン 東京), on February 21, 2026.  Doodle Duel is a real-time, competitive web party game that reinvents rock-paper-scissors as an escalating creative arms race. Players draw objects on a shared canvas within a time limit, and the game leverages the Gemini API (Vision + LLM) to classify the drawings, detect text violations, and act as an AI referee to determine which object is stronger. The strongest object becomes the new baseline to beat in the following round. The backend is built with Node.js, Express, and Socket.IO for real-time WebSocket state synchronization. The frontend is a snappy React application powered by Vite and Tailwind CSS. The entire system is containerized with Docker and reverse-proxied using Traefik. Also, an `enhanced` version of player's drawing is also generated by ImageGen 4 to add more fun and creativity to the game.",
      tools: [
        'React',
        'TypeScript',
        'Tailwind CSS',
  ...
  vi: [
    {
      id: 11,
      name: 'Doodle Duel (Jankenpon): Trò chơi Leo thang',
      description:
        'Đây là solution của chúng tôi cho Gemini 3 Hackathon Tokyo (Gemini 3 ハッカソン 東京), vào ngày 21 tháng 2 năm 2026. Doodle Duel là một trò chơi tiệc web cạnh tranh theo thời gian thực, tái tạo lại trò oẳn tù tì thành một cuộc chạy đua sáng tạo leo thang. Người chơi vẽ các vật thể trên bảng vẽ chung trong giới hạn thời gian, và trò chơi tận dụng Gemini API (Vision + LLM) để phân loại bản vẽ, phát hiện vi phạm văn bản, và đóng vai trọng tài AI để xác định vật thể nào mạnh hơn. Vật thể mạnh nhất trở thành mốc mới cần phải đánh bại trong vòng tiếp theo. Backend được xây dựng bằng Node.js, Express, và Socket.IO cho đồng bộ hóa trạng thái WebSocket thời gian thực. Frontend là ứng dụng React nhanh được hỗ trợ bởi Vite và Tailwind CSS. Toàn bộ hệ thống được container hóa bằng Docker và reverse-proxy bằng Traefik. Ngoài ra, phiên bản `nâng cao` của bản vẽ người chơi cũng được tạo ra bởi ImageGen 4 để tăng thêm niềm vui và sáng tạo cho trò chơi.',
      tools: [
        'React',
        'TypeScript',
        'Tailwind CSS',
