...
// Helper function to get icon based on news type
export const getNewsIcon = (title: string): string => {
  const lowerTitle = title.toLowerCase();

  // Job-related news
  if (lowerTitle.includes('left') || lowerTitle.includes('rời khỏi')) {
    return 'FaSignOutAlt';
  }
  if (lowerTitle.includes('started') || lowerTitle.includes('bắt đầu')) {
    return 'FaSignInAlt';
  }

  // Publications
  if (lowerTitle.includes('paper') || lowerTitle.includes('bài báo')) {
    return 'FaFileAlt';
  }
  ...
  if (
    ...
    lowerTitle.includes('đạt') ||
    lowerTitle.includes('top 1')
  ) {
    return 'FaTrophy';
  }

  // Graduation
  if (lowerTitle.includes('graduated') || lowerTitle.includes('tốt nghiệp')) {
    return 'FaGraduationCap';
  }

  // Thesis defense
  if (lowerTitle.includes('thesis') || lowerTitle.includes('khóa luận')) {
    return 'FaBookOpen';
  }
